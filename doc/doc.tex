...
%\section{Bloating}
%When two fills share an exact boundary and they are overlapping another fill which goes through that boundary, there is sometimes a visual artifact where the color of the below fill is seen as a thin line. To get over this visual artifact, all features can be slightly bloated to go over their boundaries in a way that is otherwise imperceptible. This bloating is the same process as creating a conformal layer. There must also be looseness in the bounding box.
